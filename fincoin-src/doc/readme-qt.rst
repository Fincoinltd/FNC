FinCoin-qt: Qt4 GUI for FinCoin
===============================
...
Alternatively, install Qt Creator and open the `FinCoin-qt.pro` file.

An executable named `FinCoin-qt` will be built.
...
A warning for people using the *static binary* version of FinCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of FinCoin is linked against libdb5.3.
...
installed causes FinCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
