...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FinCoinTEST2\r");
            Send(hSocket, "WHO #FinCoinTEST2\r");
        } else {
            // randomly join #FinCoin00-#FinCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #FinCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FinCoin%02d\r", channel_number).c_str());
        }
