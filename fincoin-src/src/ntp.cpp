...
void ThreadNtpSamples(void* parg) {
    ...
    RenameThread("FinCoin-ntp-samples");
    ...
    while (!fShutdown) {
        ...
        if (GetNodesOffset() == INT_MAX && abs64(nNtpOffset) > 40 * 60)
        {
            ...
            std::string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong FinCoin will not work properly.");
            strMiscWarning = strMessage;
            printf("*** %s\n", strMessage.c_str());
            uiInterface.ThreadSafeMessageBox(strMessage+" ", std::string("FinCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
        }
